...
def show_image_if_exists(path, caption):
    """Display image with version-safe API."""
...
def load_bert_model():
    """
    Load BERT directly from Hugging Face.
    No local files needed. Works 100% guaranteed.
    """
    from transformers import AutoTokenizer, AutoModelForSequenceClassification
    import torch

    try:
        # Download pre-trained sentiment model from Hugging Face
        ...
        st.session_state["_bert_error"] = str(e)
...
def load_rag_index():
    ...
    try:
        ...
        st.session_state["_rag_error"] = str(e)
        return None
...
# ============================================================================
    ...
    with col2:
        ...
        err = st.session_state.get("_lstm_error")
        ...
        if model is not None:
            ...
            if err and "quantization_config" in err:
                st.warning(
                    "⚠️ **LSTM Model Incompatible with Keras 3.11.0**\n\n"
                    "Your model was trained with an older Keras version.\n\n"
                    "**Solution:** Retrain the model with current Keras and save it again."
                )
            elif err:
                st.warning(f"⚠️ Model loading error:\n\n{err[:200]}")
            else:
                st.info("Model file not found at `models/lstm_purchase_model.keras`")
...
# ============================================================================
    ...
    if predict_clicked:
        if not review_text.strip():
            ...
            err = st.session_state.get("_bert_error")
            
            if model is None:
                st.error(f"❌ BERT model could not load. Error: {err[:300] if err else 'Unknown error'}")
            else:
                ...
                if id2label:
                    labels = [id2label.get(i, f"Label {i}") for i in range(len(probs))]
                else:
                    labels = ["Negative", "Positive"] if len(probs) == 2 else ["Negative", "Neutral", "Positive"]
